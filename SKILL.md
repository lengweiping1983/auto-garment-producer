...
6. 生产规划：运行 `scripts/构造生产规划请求.py` 生成 `ai_production_plan_prompt.txt`，启动子 Agent 输出 `ai_production_plan.json`，其中必须包含 `garment_map`、`pre_design_self_check`、`asset_shortlist`、`pre_submit_self_audit` 和 `piece_fill_plan`。
7. 程序执行：`scripts/应用生产规划.py`、`scripts/创建填充计划.py`、`scripts/渲染裁片.py`、`scripts/时尚质检.py` 依次校验并渲染。后端只做安全修正，不替代审美决策。
8. 商业复审：运行 `scripts/构造商业复审请求.py` 生成 `ai_commercial_review_prompt.txt`。复审时优先同时传 `preview_path` 和 `preview_3m_path`；`preview_3m.png` 是把最终预览缩到 64×64 后再放大到 512×512 的三米模拟图。若没有该图，复审 AI 必须在脑中模拟 64px 远看效果。
