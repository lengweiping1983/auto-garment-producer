...
- **7a. 构造请求**：运行 `scripts/构造生产规划请求.py`，生成 `ai_production_plan_prompt.txt`。
  ...
  - 内置款式参考图位于 `references/styles/`。当前覆盖 `BFSK26308XCJ01L` 男士防晒服和 `DDS26126XCJ01L` 男士衬衫；构造请求时会根据 pieces 中的 `pattern_image` 或 `garment_type` 自动注入对应参考图，让 AI 先做标准纸样对照，再判断每个 mask 的部位、上下方向和对称关系。
