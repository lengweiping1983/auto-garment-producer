...
description: 自动化成衣生产。从主题图或面料资产出发，生成商业成衣设计简报、Neo AI 单纹理面料资产、裁片填充计划，并用 OpenCV/Pillow 确定性渲染透明裁片 PNG。生成 / 创建 T 恤、T恤、t-shirt、tee、衬衫、男士衬衫、防晒服或同义上装时必须触发。
---

# 自动化成衣生产

把主题图或面料资产转成可生产的服装裁片样品。AI 只做视觉分析、提示词和审美决策；最终裁片 PNG 必须由程序渲染。用户提供主题图时，程序会让 AI 独立生成完整不裁头的透明主图，并生成前片连续资产；默认生成 3 张单纹理（main/secondary/accent_light），每张纹理分别生成一套 `preview.png`。
...
3. 设计简报：运行 `scripts/生成设计简报.py`，生成 `texture_prompts.json`。
4. 资产生成：没有 `--texture-set` 时，先把用户主题图上传为 Neo AI 参考图 URL，再优先提交透明主图任务，随后并行提交 3 张独立单纹理任务。
5. 裁片准备：固定复用内置模板资产。
6. 主题前片：优先使用 AI 透明主图生成 `theme_front_full.png`，同时保留 `theme_front_left.png` 与 `theme_front_right.png` 作为兼容资产，并注册到 `texture_set.json`；渲染时左前片与右前片必须先作为一张虚拟前身画布整体铺底纹和主图，再按各自 mask 裁回单片，纸样预览中的白色间距不得参与对位。
7. 填充计划：`scripts/创建填充计划.py` 使用规则引擎自动分配纹理到裁片，生成 `piece_fill_plan.json`。
8. 渲染：`scripts/渲染裁片.py` 生成填充计划和透明裁片；3 个纹理分别生成 3 套单纹理 9 裁片模板，直接输出在 `variants/<texture_id>/`。每套变体输出裁片 PNG、`preview.png`、`preview_white.jpg` 和清单。
...
10. **变体主图规则**：每套单纹理变体仍必须保留主题前片连续资产。`theme_front_full` 是优先主图，`theme_front_left/right` 只作兼容回退；左右前片的底纹和主图必须跨中缝连续对齐，不能分别独立落位。单纹理预览仅将底纹统一为当前纹理，不得移除主题前片 overlay。预览图只作为结果文件，不再进入 AI 输入、复审或自动检查流程。
...
- 使用 `--reuse-cache` 后，视觉分析、设计简报和可复用程序产物会按输入 hash 复用。
- 模板部位与分区事实以模板 `garment_map` 和 `piece_overview` 为准。
- 已存在并匹配输入的 `visual_elements.json`、`texture_set.json`、模板 pieces 和 mask 不重复生成。
...
- 填充计划 `piece_fill_plan.json`：包含 `garment_map.pieces[]` 和 `piece_fill_plan.pieces[]`。模板模式下 `garment_map` 由固定模板提供。
