...
把主题图或面料资产转成可生产的服装裁片样品。AI 只做视觉分析、提示词和审美决策；最终裁片 PNG 必须由程序渲染。用户提供主题图时，程序会让 AI 独立生成完整不裁头的透明主图，并生成可选的前片切半资产；最终 `preview.png` 默认只使用一张图案纹理铺满整套模板，保持所有裁片一致。
...
4. 资产生成：没有 `--texture-set` 或 `--collection-board` 时，调用 Neo AI 并行生成 2x2 面料纹理看板和 1 张透明主图。
5. 裁片准备：固定复用内置模板资产。
...
8. 程序执行：`scripts/应用生产规划.py`、`scripts/创建填充计划.py`、`scripts/渲染裁片.py` 依次生成填充计划和透明裁片；2x2 裁出的 4 个纹理全部分别生成 4 套单纹理 9 裁片模板，`main` 变体同步到默认 `rendered/`。每套 `rendered` 输出裁片 PNG、`preview.png`、`preview_white.jpg` 和清单。
9. **变体展示规则**：4 套单纹理变体（main/secondary/accent_light/accent_mid）生成后，**不得向用户展示**。stdout 仅提示“已生成 N 套单纹理变体至 variants/ 目录，不在此处展示”；`automation_summary.json` 仍保留完整变体路径供下游程序使用，但面向用户的输出必须隐藏变体预览图和详细路径。
