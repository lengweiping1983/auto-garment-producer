...
8. 程序执行：`scripts/应用生产规划.py`、`scripts/创建填充计划.py`、`scripts/渲染裁片.py` 依次生成填充计划和透明裁片；3 个纹理分别生成 3 套单纹理 9 裁片模板，`main` 变体同步到默认 `rendered/`。每套 `rendered` 输出裁片 PNG、`preview.png`、`preview_white.jpg` 和清单。
