...
把主题图或面料资产转成可生产的服装裁片样品。AI 只做视觉分析、提示词和审美决策；最终裁片 PNG 必须由程序渲染。用户提供主题图时，程序会把第一张主题图的主体裁剪后竖向切成左右两半，并强制落到正面两片衣服上。
...
4. 资产生成：没有 `--texture-set` 或 `--collection-board` 时，调用 Neo AI 生成 3x3 面料看板。
5. 裁片准备：固定复用内置模板资产。
