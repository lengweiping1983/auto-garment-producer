...
0. **🔴 子 Agent 视觉元素提取（当提供主题图时，不可跳过；但有纯 CV 离线 fallback）**
   - **推荐路径（LLM 视觉分析）**：
   ...
   - **离线 fallback 路径（纯传统图像处理，零 LLM / 零语义分割）**：
     - 当无网络、无 API token、或需要 100% 可复现结果时，运行 `scripts/纯CV视觉元素提取.py --theme`。
     - 技术栈：MedianCut 颜色量化、边缘密度网格分析、局部方差纹理检测、HSL 色彩分类。
     - 输出 `visual_elements_cv.json`，格式与 LLM 版本兼容，可直接被 `生成设计简报.py --visual-elements` 复用。
